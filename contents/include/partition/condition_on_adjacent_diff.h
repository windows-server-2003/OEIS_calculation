...
struct A325535 {
	template<typename T> std::vector<T> calc_all(int n) {
		std::vector<std::vector<T> > dp(n + 1); // dp[i][j]: number of partition of i into j positive numbers
		for (int i = 0; i <= n; i++) dp[i].resize(i + 1);
		dp[0][0] = 1;
		// either the partition contains 1, or subtract 1 from all -> partition i - j into j positive numbers
